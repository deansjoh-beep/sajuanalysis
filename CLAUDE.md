...
- `db/purge.ts` — shared purge logic. `DELETE /api/code?code=` (or legacy `/api/purge`, kept via rewrite) hard-deletes a code and cascades to orders/reports (irrecoverable by design); `GET /api/code` with `Authorization: Bearer CRON_SECRET` deletes expired reports (daily Vercel cron). Hobby cron is once-daily, so read paths must also check `expires_at` themselves.
- Saju code system (Phase 2-3): `db/code.ts` — `lookupCode` (재열람: valid reports with content, expired → `regenerable` no-charge flag, `newYearDiscountPercent` 10 for repeat buyers, per-order `followupRemaining`), `redeemGiftCode` (fills `myeongsik` on unused gift codes, atomic `WHERE myeongsik IS NULL`), `consumeFollowup` (3 per order, atomic decrement), `saveReport` (persists generated report body, flips order to `generated`; re-save allowed only after expiry). All exposed through the single `api/code.ts` function (`GET ?code=` lookup / `POST /api/code/redeem|followup|save-report` via rewrite / `DELETE` purge / `GET`+cron-secret cleanup).
- Admin ops (Phase 2-5): `db/admin.ts` — `getAdminStats` (daily revenue/refunds + generation success rate, avg `generation_cost_krw`, validation-fail rate = `quality_score` < 80, review tallies), `sampleReportsForReview` (today's unexpired unreviewed, random 10), `saveReview` (approve/reject + tags, one row per report via upsert; `report_reviews` has NO FK so review history survives report purge). Endpoints live inside existing functions: `GET /api/payment/stats`, `GET /api/code?adminSample=1`, `POST /api/code/review` — all require `x-admin-token` = `ADMIN_ACCESS_TOKEN` (unset ⇒ admin APIs disabled). UI: `SalesReviewPanel` in AdminPage (`매출·검수` section; token kept in sessionStorage).
- Beta infra (Phase 3-2): `db/feedback.ts` — `submitFeedback` (rating 1–5 + multiple-choice answers validated against `FEEDBACK_QUESTIONS` allow-list + free text; one row per code+product, upsert) and `getFeedbackStats` (rating/answer distributions, recent comments — surfaced in `/api/payment/stats` as `feedbackStats` and rendered in SalesReviewPanel). `feedback` table has no FK so stats survive code purge. Endpoint: `POST /api/code/feedback`. Beta codes: `npx tsx scripts/issue-beta-codes.ts --count 300 --product yearly2026` (`db/beta.ts` — unused gift-style code + ₩0 paid order; set `DATABASE_URL` to target Neon, otherwise local PGlite). 문항·선택지는 OWNER 확정 전 임시안 — 바꿀 땐 `db/feedback.ts`와 `CodeLookupTab.tsx` 양쪽을 동기화.
