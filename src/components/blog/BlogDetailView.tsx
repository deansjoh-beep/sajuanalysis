...
export const BlogDetailView: React.FC<BlogDetailViewProps> = ({ post, glassPanelStrongClass, onBackToList }) => {
  return (
    <motion.div
      ...
      className="space-y-8"
    >
      <button onClick={onBackToList} className="flex items-center gap-2 text-indigo-600 font-bold text-sm mb-4 hover:underline transition-all group">
      ...
      <div className={`${glassPanelStrongClass} rounded-[3rem] overflow-hidden`}>
        <div className="relative h-64 md:h-[30rem]">
          ...
          <div className="absolute bottom-8 left-8 right-8 space-y-4">
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 rounded-full bg-indigo-500 text-white text-[10px] font-bold uppercase tracking-wider shadow-lg">{post.category}</span>
              <span className="text-xs text-white/70 font-medium">{post.date}</span>
            </div>
        ...
        <div className="p-8 md:p-16 space-y-10">
          ...
          <div className="pt-10 border-t border-white/60 flex items-center justify-between">
            ...
            <button onClick={onBackToList} className="px-6 py-2 min-h-[44px] rounded-xl bg-white/70 backdrop-blur border border-white/60 text-xs font-bold hover:bg-white/85 transition-colors">
              목록보기
            </button>
          </div>
        </div>
