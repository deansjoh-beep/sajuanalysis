...
const App: React.FC = () => {
  ...
  const runAnalysis = async (data: UserData) => {
    ...
    try {
      ...
      setAiSuggestionRequestCount(0);
      preservedChatContextRef.current = [];
      // 빈 상태로 두면 상담 탭 진입 시 오프닝(명식 카드 + 시나리오 버튼)이 인사를 대신한다.
      setMessages([]);
    } catch (err: any) {
      ...
      <div className={`w-full h-full md:h-screen text-[#1a1a1a] overflow-hidden shadow-2xl relative flex flex-col transition-all duration-300 font-sans ${GLASS_TAB_BG_CLASS}`}>
        ...
        <main className="flex-1 overflow-hidden flex flex-col relative">
          ...
          {activeTab === "chat" && (
            <ChatTabContent
              consultationMode={consultationMode}
              switchConsultationMode={switchConsultationMode}
              sajuResult={sajuResult}
              yongshinResult={yongshinResult}
              userName={userData.name}
              messages={messages}
              ...
              aiSuggestionRequestCount={aiSuggestionRequestCount}
              basicSelectedCategory={basicSelectedCategory}
              setBasicSelectedCategory={setBasicSelectedCategory}
