...
## 제5조 (유료 서비스 및 결제)
1. 서비스는 유료 사주 리포트(평생 사주 리포트, 일년운세 리포트 등)를 판매하며, 각 상품의 가격은 구매 화면에 표시된 금액을 따릅니다.
2. 결제는 전자결제대행사(토스페이먼츠)를 통해 처리되며, 결제 완료 시 리포트 열람에 사용되는 사주 코드가 발급됩니다.
3. 생성된 리포트의 열람 기간은 생성 시점부터 72시간이며, 열람 기간이 지나면 본문은 파기됩니다. 동일 상품은 추가 비용 없이 재생성을 요청할 수 있습니다.
4. 결제 취소 및 환불은 별도의 취소·환불 정책을 따릅니다.
...
- 고객센터(전화·이메일)로 사주 코드와 함께 환불을 요청해 주세요.

## 2. 리포트 생성 후 (환불 불가)
- 유료 리포트는 구매자의 사주 정보를 바탕으로 개별 생성되는 **맞춤형 디지털 콘텐츠**입니다.
- 「전자상거래 등에서의 소비자보호에 관한 법률」 제17조 제2항에 따라, **리포트 생성(콘텐츠 제공)이 완료된 후에는 청약철회(환불)가 불가합니다.**
- 단순 변심, 해석 내용이 기대와 다르다는 사유는 환불 대상이 아닙니다.
- 이 제한은 구매 화면에 사전 고지되며, 결제 진행 시 동의한 것으로 봅니다.

## 3. 하자가 있는 경우
다음의 경우에는 생성 완료 후에도 **재생성 또는 전액 환불**해 드립니다.
- 리포트가 정상적으로 생성·표시되지 않는 경우
- 본문이 누락되거나 결제한 상품과 다른 내용이 제공된 경우
- 기타 서비스 오류로 정상적인 열람이 불가능한 경우

## 4. 열람 기간 만료
- 리포트 열람 기간(생성 후 72시간)이 지나 본문이 파기된 경우, 동일 상품은 **추가 비용 없이 재생성**해 드립니다. 이는 환불 사유에 해당하지 않습니다.

## 5. 환불 처리
- 환불은 결제에 사용한 수단으로 처리되며, 영업일 기준 3~5일이 소요될 수 있습니다.
- 문의: 010-2037-5296 / [dean.uitrading@gmail.com](mailto:dean.uitrading@gmail.com)`;
